/*@(#)MergedSingleSignOutHandler.java   2015-12-23 
...
/**
 * 由于shiro对cas的单点登出支持仅限于使用web应用的session缓存授权信息方式，<br/>
 * 无法支持使用ehcache等外部缓存进行授权信息缓存的模式，因此，改造cas的SingleSignOutHandler，支持同时处理cas和shiro的登出处理
 * <br/>由于cas client的SingleSignOutHandler为final，无法被继承并重写相关方法，因此在此Class重写相关处理
 * <p>
 * @version 1.0.0,2015-12-23
 * @author lw
 * @since 1.0.0
 */
public class MergedSingleSignOutHandler {
    ...
    private void shiroLogout(){
    	...
    	logger.debug("logging out shiro Subject:{}",sub.getPrincipal());
    	sub.logout();
    	logger.debug("logging out shiro Subject finished:{}",sub.getPrincipal());
    }
